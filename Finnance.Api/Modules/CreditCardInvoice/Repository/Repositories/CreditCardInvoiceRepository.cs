...
public class CreditCardInvoiceRepository : BaseRepository<CreditCardInvoiceEntity, CreditCardInvoiceMod>, ICreditCardInvoiceRepository
{
    ...
    public CreditCardInvoiceEntity SearchByCardMonth(long card, long user, string monthKey)
    {
        ...
        var sql = $"""
                   ...
                   WHERE card = @card AND "user" = @user AND month_key = @month_key{GetTenantClause()}
                   LIMIT 1
                   """;
    ...
    public List<CreditCardInvoiceEntity> SearchByCardYear(long card, long user, int year, int limit = 0, int offset = 0)
    {
        ...
        var sb = new StringBuilder();
        sb.Append("SELECT ci.*, ");
        sb.Append($"""(SELECT COUNT(*) FROM "transaction" t WHERE t.invoice = ci.credit_card_invoice AND t.active = TRUE{GetTenantClause("t")}) AS items_count """);
        sb.Append("FROM credit_card_invoice ci ");
        sb.Append("""WHERE ci.card = @card AND ci."user" = @user """);
        sb.Append(GetTenantClause("ci"));
        TenantParams(param);
    ...
    public void UpdateTotals(long creditCardInvoice, long user, decimal totalAmount, decimal paidAmount, long invoiceStatus, DateTime? paidAt)
    {
        ...
        var sql = $"""
                   ...
                   SET total_amount = @total_amount,
                       ...
                       paid_at = @paid_at,
                       updated = NOW()
                   WHERE credit_card_invoice = @credit_card_invoice AND "user" = @user{GetTenantClause()}
                   """;
    ...
    public (decimal Total, decimal Paid) SumInvoiceAmounts(long invoice, long user)
    {
        ...
        var sql = $"""
                   ...
                   FROM "transaction"
                   WHERE invoice = @invoice AND "user" = @user AND active = TRUE{GetTenantClause()}
                   """;
    ...
    public List<long> SearchTransactionIdsToReprocess(long card, long user, DateTime fromDate)
    {
        ...
        var sql = $"""
                   ...
                   WHERE card = @card AND "user" = @user AND active = TRUE{GetTenantClause()}
    ...
    public List<long> SearchInvoiceIdsByCard(long card, long user)
    {
        ...
        var sql = $"""SELECT credit_card_invoice FROM credit_card_invoice WHERE card = @card AND "user" = @user{GetTenantClause()}""";
    ...
    public List<long> SearchReferencedInvoiceIds(long card, long user)
    {
        ...
        var sql = $"""
                   ...
                   WHERE card = @card AND "user" = @user AND invoice IS NOT NULL{GetTenantClause()}
                   """;
    ...
    public void DeleteUnreferenced(long card, long user)
    {
        ...
        var sql = $"""
                   ...
                   WHERE ci.card = @card AND ci."user" = @user{GetTenantClause("ci")}
                     AND NOT EXISTS (
    ...
    public int MarkOverdue(long user)
    {
        ...
        var sql = $"""
                   ...
                   WHERE "user" = @user AND active = TRUE{GetTenantClause()}
