...
export const dashboardService = {
    async getStats(filter?: Date | { start: string; end: string }) {
        ...
        // Fetch unpaid transactions from credit cards to calculate available limit
        ...
        // Find the first transaction date to determine if we should include initial balance
        ...
        const firstDate = firstTx?.payment_date ? startOfMonth(new Date(firstTx.payment_date + 'T12:00:00')) : null;
        let shouldIncludeInitialBalance = false;

        if (!filter) {
            shouldIncludeInitialBalance = true;
        } else if (filter instanceof Date && firstDate) {
            shouldIncludeInitialBalance = format(startOfMonth(filter), 'yyyy-MM') === format(firstDate, 'yyyy-MM');
        } else if (filter && 'start' in filter && firstDate) {
            const startRange = startOfMonth(new Date(filter.start));
            shouldIncludeInitialBalance = startRange <= firstDate; // Or some logic that checks if first month is in range
        }
        ...
        return {
            totalBalance,
            totalLimit: totalLimit - totalUsed, // Show available limit
            monthlyIncome,
            monthlyExpenses,
    ...
    async getChartData(filter?: Date | { start: string; end: string }) {
        ...
        if (filter instanceof Date) {
            ...
            const start = firstTx?.payment_date ? new Date(firstTx.payment_date + 'T12:00:00') : subMonths(new Date(), 11);
            const end = new Date();
        ...
        (transactions as any[])?.forEach(t => {
            ...
            if (month) {
                ...
                } else if ((t.type === 'expense' || t.type === 'despesa') && !t.card_id) {
                    month.despesa += Number(t.amount);
                }
            }
        });
