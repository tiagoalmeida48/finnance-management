...
import type { CardCategoryPoint, CardHistoryChartPoint } from '@/shared/interfaces/card-details.interface';
...
export function CardDetailsCharts({ chartData, categoryData }: CardDetailsChartsProps) {
    ...
    return (
        <Container unstyled className="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-[2fr_1fr]">
            ...
            <Container unstyled className="group relative overflow-hidden rounded-[24px] border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
                <Container unstyled className="mb-4 flex items-center justify-between">
                    <Text className="font-heading text-lg font-bold text-white tracking-tight">{chartMessages.spendingEvolution}</Text>
                    <Container unstyled className="flex items-center gap-2 rounded-full border border-[var(--color-primary)]/20 bg-[var(--color-primary)]/10 px-3 py-1">
                        <Container unstyled className="h-2 w-2 rounded-full bg-[var(--color-primary)] shadow-[0_0_8px_var(--color-primary)]" />
                        <Text className="text-xs font-bold uppercase tracking-wider text-[var(--color-primary)]">{chartMessages.total}</Text>
                    </Container>
                </Container>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                            ...
                            <Tooltip
                                ...
                                formatter={(value: number | string | undefined) => [formatCurrency(Number(value) || 0), chartMessages.total]}
                            />
                            {average > 0 ? (
                                <ReferenceLine
                                    y={average}
                            ...
                            <Area
                                ...
                                activeDot={{ r: 6, fill: colors.bgPrimary, stroke: 'var(--color-primary)', strokeWidth: 2 }}
                            />
                        </AreaChart>
                    </ResponsiveContainer>
                </Container>
            ...
            <Container unstyled className="group relative overflow-hidden rounded-[24px] border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
                <Text className="font-heading mb-4 text-lg font-bold text-white tracking-tight">{chartMessages.categories}</Text>
                ...
                <Container unstyled className="relative flex h-[210px] min-w-0 w-full justify-center">
                    ...
                    <Container unstyled className="pointer-events-none absolute inset-0 z-[1] flex items-center justify-center">
                        ...
                        >
                            <Container unstyled className="mb-1.5 flex items-center justify-between">
                                <Container unstyled className="flex items-center gap-2">
                                    <Container unstyled className="h-3 w-3 rounded-full bg-current shadow-[0_0_8px_currentColor]" />
                                    <Text className="text-[13px] font-bold tracking-tight text-white group-hover/pill:text-current transition-colors">{category.name}</Text>
                                </Container>
                            ...
                            <Container unstyled className="flex items-center gap-3">
                                <Container unstyled className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/5 shadow-inner">
                                    <Container unstyled
