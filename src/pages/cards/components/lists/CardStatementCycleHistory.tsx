...
import { Table, TableBody, TableHead, TableHeaderCell, TableRow } from '@/shared/components/layout/Table';
        ...
        <>
            <Dialog open={open} onClose={closeHistoryDialog} fullWidth maxWidth="xl">
                <DialogTitle className="pb-2">
                    <Row className="flex-col justify-between gap-2 sm:flex-row sm:items-center">
                        <Row className="items-center gap-2">
                            ...
                            <Stack>
                                <Heading level={3} className="font-heading text-lg font-bold text-[var(--color-text-primary)]">
                ...
                <DialogContent className="pt-1">
                    <Text className="mb-2 text-xs text-[var(--color-text-muted)]">{cycleHistoryMessages.description}</Text>
                    ...
                    <CollectionState
                        ...
                        loadingFallback={(
                            <Container unstyled className="flex justify-center py-4">
                                <Container unstyled className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--color-primary)] border-r-transparent" />
                            </Container>
                        )}
                        emptyFallback={(
                            <Container unstyled className="rounded-xl border border-[var(--color-border)] p-3 text-center text-sm text-[var(--color-text-muted)]">
                                {cycleHistoryMessages.empty}
                            </Container>
                        )}
                    >
                        <Container unstyled className="overflow-auto rounded-xl border border-[var(--color-border)]">
                            <Table>
                                <TableHead>
                                    <TableRow className="bg-[var(--overlay-white-02)]">
                                        <TableHeaderCell className="font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.validity}</TableHeaderCell>
                                        <TableHeaderCell className="font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.closing}</TableHeaderCell>
                                        <TableHeaderCell className="font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.due}</TableHeaderCell>
                                        <TableHeaderCell className="font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.status}</TableHeaderCell>
                                        <TableHeaderCell className="font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.notes}</TableHeaderCell>
                                        <TableHeaderCell className="text-center font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">{cycleHistoryMessages.columns.actions}</TableHeaderCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
            ...
            <FormDialog
                ...
                maxWidth="md"
                title={dialogMode === 'edit' ? cycleHistoryMessages.editTitle : cycleHistoryMessages.createTitle}
                onSubmit={submitForm}
            ...
            >
                <Stack className="gap-2">
                    <Text className="text-xs text-[var(--color-text-muted)]">
                        {dialogMode === 'edit' ? cycleHistoryMessages.editHint : cycleHistoryMessages.createHint}
                    </Text>

                    {dialogMode === 'create' ? (
                        <FormField label={cycleHistoryMessages.fields.startDate} errorMessage={errors.date_start?.message}>
                            <Input
                                type="date"
                                {...register('date_start')}
                    ...
                    <Container unstyled className="grid gap-2 sm:grid-cols-2">
                        <FormField label={cycleHistoryMessages.fields.closingDay} errorMessage={errors.closing_day?.message}>
                            <Input
                                type="number"
                                min={1}
                                max={31}
                        ...
                        <FormField label={cycleHistoryMessages.fields.dueDay} errorMessage={errors.due_day?.message}>
                            <Input
                                type="number"
                                min={1}
                                max={31}
            ...
            <Dialog open={deleteConfirmId !== null} onClose={closeDeleteDialog} maxWidth="xs">
                ...
                <DialogActions className="gap-2">
                    ...
                    <Button onClick={handleDelete} disabled={isMutating} variant="contained" color="error">
                        {isDeleting ? cycleHistoryMessages.actions.deleting : cycleHistoryMessages.actions.confirm}
                    </Button>
                </DialogActions>
            </Dialog>
        </>
