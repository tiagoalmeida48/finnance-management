...
import type { StatementSortDirection, StatementSortField } from '@/pages/cards/hooks/useCardStatementListLogic';
import { formatCardStatementCurrency, getStatementDisplayDateKey } from '@/pages/cards/hooks/useCardStatementListLogic';
...
function StatementDesktopRow({
    ...
    return (
        <TableRow onClick={() => onEditTransaction?.(transaction)} className="cursor-pointer transition-colors hover:bg-white/5">
            <TableCell>{displayDate ? format(new Date(`${displayDate}T12:00:00`), 'dd/MM') : '-'}</TableCell>
            ...
            <TableCell>
                <Row className="items-center gap-1">
                    <Container unstyled ref={(node) => setCategoryDotRef(node, categoryColor)} className="h-2 w-2 rounded-sm" />
