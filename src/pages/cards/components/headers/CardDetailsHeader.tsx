...
export function CardDetailsHeader({
    ...
    return (
        <Container unstyled className="mb-6 flex flex-col gap-4">
            {/* Top Navigation */}
            <Container unstyled className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
                <Button
                ...
                <Container unstyled className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
                    <Button
                        size="small"
                    ...
                    <Container unstyled className="inline-flex items-center rounded-full border border-white/5 bg-white/[0.03] p-1 shadow-sm backdrop-blur-sm">
                        <button
                            type="button"
                    ...
                    >
                        <IconButton size="small" onClick={handlePrevYear} className="h-7 w-7 text-[var(--color-text-muted)] transition-colors hover:bg-white/10 hover:text-white">
                            <ChevronLeft size={16} />
                        </IconButton>
                        <Text as="span" className="min-w-[50px] text-center text-[13px] font-bold tracking-widest text-[#D4AF37]">
                            {selectedDate.getFullYear()}
                        </Text>
                        <IconButton size="small" onClick={handleNextYear} className="h-7 w-7 text-[var(--color-text-muted)] transition-colors hover:bg-white/10 hover:text-white">
            ...
            <Container unstyled className="relative overflow-hidden rounded-[24px] border border-white/5 bg-[var(--color-card)] p-6 shadow-sm">
                ...
                <Container unstyled className="relative z-10 flex flex-col items-center gap-5 sm:flex-row">
                    ...
                    <Container unstyled ref={cardIconRef} className="relative flex h-16 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[14px] shadow-lg">
                        ...
                        <CardIcon size={24} className="relative z-10 ml-4 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]" />
                    </Container>

                    <Container unstyled className="text-center sm:text-left">
                        <Text className="font-heading text-[28px] font-black tracking-tight text-white">{card.name}</Text>
                        <Text className="mt-1 flex flex-wrap items-center justify-center gap-2 text-[13px] text-[var(--color-text-secondary)] sm:justify-start">
                            <span>{headerMessages.ending} <strong className="text-white">{dueDay}</strong></span>
                            <span>•</span>
                            <span>{headerMessages.closing} <strong className="text-white">{closingDay}</strong></span>
                        </Text>
                    </Container>
                </Container>
            </Container>
...
function HeaderMetricCard({ icon: Icon, iconColor, iconClassName, label, value, valueClass, progress }: HeaderMetricCardProps) {
    return (
        <Container unstyled className="group relative overflow-hidden rounded-[20px] border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
            <Container unstyled className="mb-3 flex items-center gap-2">
                <Container unstyled className="flex h-6 w-6 items-center justify-center rounded-lg bg-[var(--color-background)]">
                    <Icon size={12} color={iconColor} className={iconClassName} />
                </Container>
                <Text as="span" className="text-[12px] font-bold tracking-wide text-[var(--color-text-secondary)]">{label}</Text>
            </Container>
            <Text className={`font-heading text-[22px] font-black tracking-tight ${valueClass}`}>{value}</Text>

            {progress !== undefined && (
                <Container unstyled className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-white/10 shadow-inner">
                    <div
                        className={`h-full rounded-full transition-all duration-1000 ${progress >= 80 ? 'bg-[var(--color-error)]' : progress >= 50 ? 'bg-[var(--color-warning)]' : 'bg-[var(--color-success)]'
