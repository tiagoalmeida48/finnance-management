...
import type { CardStatement, StatementTransaction } from '@/shared/interfaces/card-details.interface';
import type { StatementSortDirection, StatementSortField, StatementStatusBadge } from '@/pages/cards/hooks/useCardStatementListLogic';
...
export function CardStatementMonthCard({
    ...
    return (
        <Container unstyled className="group/card relative overflow-hidden rounded-[20px] border border-[var(--color-border)] bg-[var(--color-card)] shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
            <Button type="button" variant="text" size="small" onClick={onToggleExpand} className="flex w-full items-center p-4 text-left transition-colors hover:bg-white/5 data-[state=open]:bg-white/5">
                <Container unstyled className="grid w-full items-center gap-3 md:grid-cols-12">
                    <Container unstyled className="md:col-span-4">
                        <Row className="items-center gap-3">
                            <Container unstyled className="flex h-8 w-8 items-center justify-center rounded-full bg-white/5 text-[var(--color-text-secondary)] transition-transform duration-300 group-hover/card:bg-white/10 group-hover/card:text-white">
                                {isExpanded ? (
                                    <ChevronDown size={18} />
                                ) : (
                            ...
                            <Text as="span" className={`rounded-md px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider backdrop-blur-sm ${status.className}`}>
                                {status.label}
                            </Text>
                            {!isMobile ? (
                    ...
                    <Container unstyled className="md:col-span-8">
                        <Row className="items-center justify-between gap-3 md:justify-end">
                            ...
                            <Row className="items-center gap-4">
                                <Text as="span" className={`font-heading text-[18px] font-black tracking-tight ${statement.unpaidTotal > 0 ? 'text-white' : 'text-[var(--color-success)] drop-shadow-[0_0_8px_rgba(0,200,83,0.3)]'}`}>
