import { BarChart3, CreditCard as CardIcon, Pencil, Trash2 } from 'lucide-react';
            ...
            <CardContent className="p-6">
                <Row className="mb-3 items-start justify-between">
                    <Row className="items-center gap-2">
                        <Container unstyled className="flex h-11 w-11 items-center justify-center rounded-xl bg-[color:color-mix(in_oklab,var(--card-color)_20%,transparent)] text-[var(--card-color)]">
                            <CardIcon size={20} />
                        </Container>
                        <Stack>
                            ...
                            <Text className="text-[13px] text-[var(--color-text-secondary)]">
                                ...
                                <Text as="span" className="font-semibold">{dueDay}</Text>
                                {' • '}
                                {cardMessages.closeShort}:{' '}
                                <Text as="span" className="font-semibold">{closingDay}</Text>
                            </Text>
                        </Stack>
                    </Row>
                ...
                <Stack className="mb-3 space-y-0">
                    ...
                    <Row className="mb-1 items-baseline gap-1">
                        <Text className="font-heading text-[22px] font-bold text-[var(--color-text-primary)]">{usageLabel}</Text>
                    ...
                    <Container unstyled className="flex items-center gap-1.5 text-[var(--progress-color)]">
                        <Container unstyled className="h-2 flex-1 overflow-hidden rounded bg-white/5">
                            <Container unstyled ref={usageBarRef} className="h-full rounded bg-current transition-all duration-300" />
                        </Container>
                        <Text className="min-w-[45px] text-right text-xs font-semibold text-current">{usagePercentLabel}</Text>
                    </Container>
                </Stack>

                <Grid className="grid-cols-2 gap-1.5">
                    {summaryItems.map((item) => (
                        <Container unstyled key={item.key} className="rounded-[10px] bg-[var(--overlay-white-03)] p-[7px]">
                            <Text className="mb-0.5 text-[11px] font-medium uppercase text-[var(--color-text-muted)]">
                                {item.label}
                            </Text>
                            <Text className={`font-heading text-base font-bold ${item.valueClass}`}>{item.value}</Text>
                        </Container>
                    ))}
                </Grid>
            </CardContent>
