...
import { useCreditCardDetails } from "@/shared/hooks/api/useCreditCards";
import type {
  CardCategoryPoint,
...
export function useCreditCardDetailsLogic() {
  ...
  const [selectedStatement, setSelectedStatement] =
    useState<CardStatement | null>(null);
  ...
  const historyData = useMemo(() => {
    ...
    for (const t of cardTransactions) {
      ...
      existing.push({
        ...
        statementDate: new Date(
          `${invoices.find((inv) => inv.id === t.invoice_id)?.month_key ?? format(new Date(), "yyyy-MM")}-01T12:00:00`,
        ),
        statementMonthKey:
          invoices.find((inv) => inv.id === t.invoice_id)?.month_key ?? "",
      });
    ...
    if (!isAllTime) {
      ...
      filteredInvoices = invoices.filter((inv) =>
        inv.month_key.startsWith(`${selectedYearKey}-`),
      );
    }
