...
import { CreditCard as CardIcon, Percent, TrendingDown, Wallet } from 'lucide-react';
...
export function useCardDetailsHeaderLogic({
    ...
    const usageColorClass = usagePercent >= 80
        ? 'text-[var(--color-error)]'
        : usagePercent >= 50
            ? 'text-[var(--color-warning)]'
            : 'text-[var(--color-success)]';

    const cardShadowColor = card.color ? `${card.color}40` : 'var(--color-accentGlow)';
    ...
    const metrics = useMemo<any[]>(() => ([
        ...
        },
        {
            ...
            valueClass: (card.available_limit || 0) < 0 ? 'text-[var(--color-error)]' : 'text-[var(--color-success)]',
        },
        {
            key: 'utilization',
            icon: Percent,
    ...
    ]), [card.available_limit, card.credit_limit, card.usage, headerMessages.available, headerMessages.totalLimit, headerMessages.usedLimit, headerMessages.utilization, usageColorClass, usagePercent]);
