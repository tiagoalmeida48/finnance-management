...
export type StatementSortField =
  | "payment_date"
  | "description"
  | "category"
  | "amount";
...
export function useCardStatementListLogic({
  cardId,
  statements,
}: UseCardStatementListLogicParams) {
  const statementMessages = messages.cards.statementList;
  const isMobile = useMediaQuery("(max-width: 899px)");
  const [expandedMonth, setExpandedMonth] = useState<string | null>(null);
  const [statementSortField, setStatementSortField] =
    useState<StatementSortField>("payment_date");
  ...
  const setCategoryDotRef = useCallback(
    (node: HTMLDivElement | null, color: string) => {
      if (!node) return;
  ...
  const setCategoryIconRef = useCallback(
    (node: HTMLDivElement | null, color: string) => {
      if (!node) return;
    ...
    (transactions: CardStatement["transactions"]) => {
      ...
      sorted.sort((left, right) => {
        ...
        if (statementSortField === "payment_date") {
          ...
          comparison = getSortableCategoryName(
            left.category?.name,
          ).localeCompare(
  ...
  const statementItems = useMemo(
    () =>
      statements.map((statement) => ({
        ...
        transactions: getSortedStatementTransactions(
