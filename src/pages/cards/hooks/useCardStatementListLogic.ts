...
const getSortableCategoryName = (categoryName: string | null | undefined) =>
    (categoryName && categoryName.trim().length > 0 ? categoryName : messages.cards.statementList.noCategory).toLowerCase();
...
export function useCardStatementListLogic({ cardId, statements }: UseCardStatementListLogicParams) {
    ...
    const handleReprocessClick = useCallback((event: MouseEvent<HTMLButtonElement>) => {
        event.preventDefault();
        event.stopPropagation();
        handleReprocess();
    }, [handleReprocess]);

    const getStatusBadge = useCallback((statement: CardStatement): StatementStatusBadge => {
        if (statement.unpaidTotal > 0) {
            return {
                label: statementMessages.statusOpen,
                className: 'bg-[var(--color-warning)]/10 text-[var(--color-warning)] ring-1 ring-inset ring-[var(--color-warning)]/20 shadow-[0_0_12px_rgba(255,166,0,0.15)]',
            };
        }
        return {
            label: statementMessages.statusPaid,
            className: 'bg-[var(--color-success)]/10 text-[var(--color-success)] ring-1 ring-inset ring-[var(--color-success)]/20 shadow-[0_0_12px_rgba(0,200,83,0.15)]',
        };
    }, [statementMessages.statusOpen, statementMessages.statusPaid]);

    const handleStatementSort = useCallback((field: StatementSortField) => {
    ...
    const getSortedStatementTransactions = useCallback((transactions: CardStatement['transactions']) => {
    ...
    const statementItems = useMemo(() => statements.map((statement) => ({
        statement,
