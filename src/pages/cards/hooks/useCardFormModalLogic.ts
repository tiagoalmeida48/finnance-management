...
export function useCardFormModalLogic({ open, onClose, card }: UseCardFormModalLogicParams) {
    ...
    const setBankAccountId = (value: string) => {
        setValue('bank_account_id', value, { shouldDirty: true, shouldValidate: true });
    };
    ...
    const handleMoneyInput = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        setValue('credit_limit', numericValue, { shouldDirty: true, shouldValidate: true });
    };

    const creditLimitDisplayValue = formatMoneyInputValue(String(Math.round(Number(creditLimit || 0) * 100)));
