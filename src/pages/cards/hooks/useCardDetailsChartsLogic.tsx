...
export function useCardDetailsChartsLogic({
    ...
    const average = useMemo(
        () => (chartData.length > 0 ? chartData.reduce((sum, item) => sum + (item.total || 0), 0) / chartData.length : 0),
        [chartData],
    );
    ...
    const setCategoryProgressRef = useCallback((node: HTMLDivElement | null, percentage: number, index: number) => {
        if (!node) return;
        ...
        return (
            ...
            >
                <Container unstyled className="mb-1 flex items-center gap-1">
                    ...
                    <Text className="text-xs font-semibold text-[var(--color-text-secondary)]">{item.name}</Text>
                </Container>
                <Container unstyled className="flex items-center justify-between gap-1.5">
                    <Text className="text-sm font-bold text-[var(--color-text-primary)]">{formatCurrency(item.value)}</Text>
