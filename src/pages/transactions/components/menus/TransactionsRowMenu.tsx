...
function MenuAction({ onClick, icon, label, actionColor = 'default', disabled }: MenuActionProps) {
    ...
    if (actionColor === 'danger') {
        ...
        colorClasses = 'text-[var(--color-success)] hover:brightness-75 hover:bg-[var(--color-success)]/10';
    } else if (actionColor === 'warning') {
        colorClasses = 'text-[var(--color-warning)] hover:brightness-75 hover:bg-[var(--color-warning)]/10';
    }
...
export function TransactionsRowMenu({
    ...
    return (
        <>
            ...
            />
            <Container unstyled ref={menuPositionRef} className={transactionsPageStyles.rowMenu}>
                <MenuAction onClick={onEdit} icon={<Pencil size={16} />} label={rowMenuMessages.edit} />
                <MenuAction onClick={onDuplicate} disabled={duplicatePending} icon={<Plus size={16} />} label={rowMenuMessages.duplicate} />
                ...
                <MenuAction onClick={onDelete} icon={<Trash2 size={16} />} label={rowMenuMessages.delete} actionColor="danger" />
