...
export function TransactionsSummary({ summaries, isLoading }: TransactionsSummaryProps) {
    ...
    const summaryCards = [
        ...
        },
        {
            ...
            icon: ArrowRightLeft,
            iconClass: summaries.balance >= 0
                ? 'bg-[var(--color-success)]/15 text-[var(--color-success)]'
                : 'bg-[var(--color-error)]/15 text-[var(--color-error)]',
            textClass: summaries.balance >= 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]',
        },
        {
    ...
    return (
        <Container unstyled className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
            {summaryCards.map((card, idx) => {
                const Icon = card.icon;
                return (
                    <Card key={idx} className="group relative cursor-default overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-bgSecondary)]/95 shadow-md shadow-black/20 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-[var(--color-border)] hover:bg-[var(--color-bgSecondary)] hover:shadow-xl hover:shadow-black/40">
                        ...
                        <CardContent className="relative p-4 md:p-5">
                            <Container unstyled className="flex flex-col gap-3">
                                ...
                                <Container unstyled>
                                    {isLoading ? (
                                        <Container unstyled className="h-8 w-2/3 animate-pulse rounded-lg bg-white/5" />
                                    ) : (
                                        <Text
