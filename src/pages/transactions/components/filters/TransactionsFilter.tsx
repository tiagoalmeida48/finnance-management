...
function TabButton({ active, onClick, children }: TabButtonProps) {
    return (
        <Button
            type="button"
            onClick={onClick}
            className={`shrink-0 rounded-lg px-3 py-1.5 text-[13px] font-medium transition-all duration-200 border ${active
...
export function TransactionsFilter({
    ...
    return (
        <Container unstyled className="mb-4 flex flex-col justify-between gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bgSecondary)]/95 p-2 shadow-lg shadow-black/40 backdrop-blur-md md:flex-row md:items-center z-10">
            <Container unstyled className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] md:pb-0">
                <Container unstyled className="flex shrink-0 gap-1 rounded-xl bg-black/40 p-1">
                    <TabButton active={typeFilter === 'income'} onClick={() => setTypeFilter(typeFilter === 'income' ? null : 'income')}>
                        {messages.transactions.filters.typeIncome}
                    </TabButton>
                    <TabButton active={typeFilter === 'expense'} onClick={() => setTypeFilter(typeFilter === 'expense' ? null : 'expense')}>
                        {messages.transactions.filters.typeExpense}
                    </TabButton>
                    <TabButton active={typeFilter === 'transfer'} onClick={() => setTypeFilter(typeFilter === 'transfer' ? null : 'transfer')}>
            ...
            <Container unstyled className="flex items-center justify-between gap-2 overflow-x-auto pb-1 md:justify-end md:pb-0">
                <Button
                    type="button"
                    onClick={() => setHideCreditCards(!hideCreditCards)}
                    className={`flex shrink-0 items-center gap-2 rounded-xl border px-3 py-1.5 transition-all duration-200 ${hideCreditCards
                        ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/10 text-[var(--color-primary)]'
                        : 'border-transparent bg-black/40 text-[var(--color-text-secondary)] hover:bg-white/5 hover:text-white'
                        }`}
                >
                    <CreditCard size={14} className={hideCreditCards ? "text-[var(--color-primary)]" : "text-[var(--color-text-muted)]"} />
                    <Text as="span" className="whitespace-nowrap text-[13px] font-medium">
                        {hideCreditCards ? messages.transactions.filters.showCreditCard : messages.transactions.filters.hideCreditCard}
                    </Text>
                </Button>

                <Container unstyled className="flex shrink-0 gap-2">
                    <Container unstyled className="flex gap-1 rounded-xl bg-black/40 p-1">
                        <TabButton active={!showAllTime && !showInstallmentsOnly} onClick={handleSelectMonthlyView}>
                            {messages.transactions.filters.monthView}
                        </TabButton>
                        <TabButton active={showAllTime && !showInstallmentsOnly} onClick={handleSelectGeneralView}>
