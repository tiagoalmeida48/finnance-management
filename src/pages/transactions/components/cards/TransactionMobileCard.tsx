...
import { TrendingUp, TrendingDown, ArrowRightLeft, MoreVertical, ChevronDown, CheckCircle2, Clock, Landmark, Circle } from 'lucide-react';
...
export function TransactionMobileCard({
        ...
        switch (type) {
            case 'income':
                return {
                    ...
                    amountClass: isPaid ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-success)]',
                };
            case 'expense':
                return {
                    icon: TrendingDown,
                    iconClass: 'bg-[var(--color-redBg)] text-[var(--color-error)]',
                    amountClass: isPaid ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-error)]',
                };
            default:
                return {
                    icon: ArrowRightLeft,
    ...
    const setCategoryChipRef = (node: HTMLSpanElement | null) => {
        if (!node) return;
        node.style.setProperty('background-color', transaction.category?.color ? `${transaction.category.color}15` : 'var(--overlay-white-05)');
        node.style.setProperty('color', transaction.category?.color || colors.textSecondary);
    };
    return (
        <Container unstyled className="relative rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-2">
                    ...
                    <Container unstyled className="flex items-center gap-2">
                        {!isGroup && !transaction.card_id && (
                            ...
                            >
                                ...
                                <Text as="span" className={`text-[11px] ${isPaid ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}>
                                    {isPaid ? messages.transactions.row.paid : messages.transactions.row.pending}
                                </Text>
                            </Button>
                        )}
                        {isGroup && (
                            <Text as="span" className="rounded bg-white/10 px-2 py-0.5 text-[10px] text-[var(--color-text-secondary)]">
                                {messages.transactions.mobileCard.paidCountLabel(group!.paidItemsCount, group!.totalItemsCount)}
                            </Text>
                        )}
                    </Container>
                </Container>
                ...
                <Container unstyled className="flex items-start gap-2">
                    ...
                    <Container unstyled className="min-w-0 flex-grow">
                        ...
                        <Container unstyled className="flex flex-wrap items-center gap-1">
                            {isBillPayment && (
                                <Text as="span" className="rounded bg-[var(--overlay-primary-14)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-primary)]">
                            ...
                            {isGroup && (
                                <Text as="span" className="rounded bg-[var(--color-purpleBg)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-secondary)]">
