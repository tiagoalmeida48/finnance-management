...
export function TransactionsTable({
    ...
    return (
        <Container unstyled className="min-h-[360px] overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-card)]">
            <Table className="w-full border-separate border-spacing-0">
                ...
                <TableBody>
                    {groupedTransactions.length === 0 && (
                        <TableRow>
                            <TableCell colSpan={9} className="py-6 text-center text-sm text-[var(--color-text-muted)]">
                        ...
                        if ('isGroup' in item && item.isGroup) {
                            ...
                            return (
                                <Fragment key={group.id}>
                                    <TableRow
                                        className="min-h-14 border-b border-white/5 border-l-[3px] border-l-[var(--color-primary)] bg-[var(--color-primary)]/5 transition-colors duration-200 hover:bg-[var(--color-primary)]/10"
                                    >
                                        <TableCell className="border-b-0 px-2 py-2">
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    const isAllSelected = group.items.every(it => selectedIds.includes(it.id));
                                                    group.items.forEach(it => {
                                                        if (!isAllSelected && !selectedIds.includes(it.id)) handleSelectRow(it.id);
                                                        if (isAllSelected && selectedIds.includes(it.id)) handleSelectRow(it.id);
                                                    });
                                                }}
                                                className={`flex h-5 w-5 items-center justify-center rounded transition-all ${group.items.some(it => selectedIds.includes(it.id))
                                        ...
                                        <TableCell className="w-[100px] border-b-0 px-2 py-2">
                                            <Text as="span"
                                                className={`inline-flex items-center justify-center rounded-md px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${group.type === 'installment'
                                        ...
                                        <TableCell className="border-b-0 px-2 py-2">
                                            <Container unstyled className="flex items-center gap-2">
                                                ...
                                                <Text as="span" className="rounded-md bg-white/10 px-2 py-0.5 text-[10px] text-white/70">
                                        ...
                                        <TableCell className="border-b-0 px-2 py-2">
                                            {group.categoryName ? (
                                                ...
                                                <Text as="span" className="inline-flex items-center justify-center rounded-md bg-white/5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-muted)]">
                                        ...
                                        <TableCell className="border-b-0 px-2 py-2">
                                            <Container unstyled className="flex min-w-[220px] items-center gap-2">
                                                <Container unstyled className="h-1.5 w-full max-w-[240px] overflow-hidden rounded-full bg-white/10">
                                                    <Container unstyled
                                        ...
                                        <TableCell className={`min-w-[100px] border-b-0 px-2 py-2 text-right text-[13px] font-bold tracking-tight ${group.isAllPaid ? 'text-[var(--color-success)]' : 'text-[var(--color-accent)]'}`}>
