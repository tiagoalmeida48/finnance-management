    ...
    return (
        <Container unstyled>
            <Container unstyled className="mb-2 flex gap-1.5">
                <Container unstyled
                    className={`flex-1 rounded-[10px] border bg-white/[0.03] p-1.5 transition-all duration-200 hover:bg-white/[0.05] ${isFixed ? 'border-[var(--color-blue)]' : 'border-[var(--overlay-white-06)]'
                        }`}
                >
                    <Container unstyled className="flex items-center justify-between relative group">
                        <Container unstyled className="flex items-center gap-1.5">
                            ...
                            <Text className={`text-[13px] font-medium ${isFixed ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)]'}`}>
                                {toggleConfig.recurring.label}
                            </Text>
                        </Container>
                        <Container unstyled className="relative flex items-center justify-center p-0.5">
                            <input {...register('is_fixed')} type="checkbox" className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" />
                            {isFixed ? (
                                <CheckCircle2 size={16} className="text-[var(--color-warning)] transition-colors group-hover:brightness-90" />
                            ) : (
                                <Circle size={16} className="text-[var(--overlay-white-30)] transition-colors group-hover:text-[var(--overlay-white-50)]" />
                            )}
                        </Container>
                    </Container>
                </Container>
                ...
                <Container unstyled
                    className={`flex-1 rounded-[10px] border bg-white/[0.03] p-1.5 transition-all duration-200 hover:bg-white/[0.05] ${isInstallment ? 'border-[var(--color-secondary)]' : 'border-[var(--overlay-white-06)]'
                        }`}
                >
                    <Container unstyled className="flex items-center justify-between relative group">
                        <Container unstyled className="flex items-center gap-1.5">
                            <InstallmentIcon size={16} color={isInstallment ? toggleConfig.installment.activeColor : colors.textMuted} />
                            <Text className={`text-[13px] font-medium ${isInstallment ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)]'}`}>
                        ...
                        {amount > 0 && totalInstallments > 0 && (
                            <Text className="mt-0.5 text-xs text-[var(--color-accent)]">
                                {messages.transactions.form.advanced.installmentsSummary(
                                    totalInstallments,
                                    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(amount / totalInstallments),
                                )}
                            </Text>
                        )}
                    </Container>
