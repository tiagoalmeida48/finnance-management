...
export function ImportTransactionsPreviewTable({
    ...
    const visibleColumns = [
        ...
        { key: 'installments' as const, label: previewMessages.columns.installments },
        ...(
            isCreditPayment
                ? []
                : [
                    { key: 'paymentDate' as const, label: previewMessages.columns.paymentDate },
                    { key: 'paymentAccount' as const, label: previewMessages.columns.paymentAccount },
                ]
        ),
    ];
    ...
    return (
        <>
            <Container unstyled className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-sm font-bold text-[var(--color-text-primary)]">{previewMessages.title}</h3>
                <ValidationChip
            ...
            <Container unstyled className="max-h-[400px] overflow-auto rounded-xl border border-[var(--color-border)]">
...
function ValidationChip({ label, color }: { label: string; color: 'success' | 'warning' }) {
    return (
        <Container unstyled
            className={`rounded-full border px-2 py-1 text-xs font-bold ${color === 'success'
