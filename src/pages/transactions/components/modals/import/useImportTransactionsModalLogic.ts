...
export function useImportTransactionsModalLogic(onClose: () => void) {
    ...
    const transformRow = useCallback((row: FileData): FileData => {
    ...
    const updateRow = useCallback((index: number, field: keyof FileData, value: string) => {
    ...
    const handlePaymentMethodChange = useCallback((nextMethod: ImportPaymentMethod) => {
    ...
    }, [parser.previewData, accounts, categories, cards, paymentMethod, selectedAccountId, selectedCardId]);
