...
const batchChangeDaySchema = z.object({
    day: z.number().int().min(1, batchChangeDayMessages.dayValidation).max(31, batchChangeDayMessages.dayValidation),
});
...
export function BatchChangeDayModal({
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
            <DialogTitle className="font-bold">{batchChangeDayMessages.title}</DialogTitle>
            <form onSubmit={handleSubmit(handleConfirm)}>
                <DialogContent>
                    <Container unstyled className="mt-1 flex flex-col gap-2">
                        ...
                        <Container unstyled className="space-y-1">
                            <Label htmlFor="batch-change-day">{batchChangeDayMessages.dayFieldLabel}</Label>
                            <Input
                                id="batch-change-day"
                                type="number"
                                min={1}
                            ...
                            />
                            {errors.day?.message && (
                                <Text className="text-xs text-red-400">{errors.day.message}</Text>
                            )}
                        </Container>
                    </Container>
                </DialogContent>
                <DialogActions className="p-3">
                    ...
                    <Button variant="contained" type="submit" disabled={loading}>
                        {loading ? commonMessages.states.applying : commonMessages.actions.apply}
                    </Button>
                </DialogActions>
            </form>
        </Dialog>
