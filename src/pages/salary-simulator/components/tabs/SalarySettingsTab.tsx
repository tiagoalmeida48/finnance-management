...
import { Table, TableBody, TableCell, TableHead, TableHeaderCell, TableRow } from '@/shared/components/layout/Table';
                    ...
                    <Container unstyled className="flex flex-col gap-2">
                        ...
                        {loadingHistory ? (
                            <Container unstyled className="flex justify-center py-4">
                                <Text as="span" className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--color-primary)] border-r-transparent" />
                            </Container>
                        ) : (
                                ...
                                <Table className="w-full text-sm">
                                    <TableHead>
                                        <TableRow className="bg-[var(--overlay-white-02)]">
                                            <TableHeaderCell className={headerCellClass}>{tabMessages.columns.validity}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-right`}>{tabMessages.columns.hourValue}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-right`}>{tabMessages.columns.proLabore}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-right`}>{tabMessages.columns.inss}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-right`}>{tabMessages.columns.adminFee}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-center`}>{tabMessages.columns.status}</TableHeaderCell>
                                            <TableHeaderCell className={`${headerCellClass} text-center`}>{tabMessages.columns.actions}</TableHeaderCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="py-1.5 text-center">
                                                        {isCurrent ? (
                                                            <Text as="span" className="inline-flex rounded bg-[var(--color-primary)]/20 px-2 py-0.5 text-xs font-semibold text-[var(--color-primary)]">
                                                                {tabMessages.current}
                                                            </Text>
                                                        ) : '-'}
                                                    </TableCell>
                                        ...
                                        {paginatedHistory.length === 0 && (
                                            <TableRow>
                                                <TableCell colSpan={7} className="py-3 text-center text-sm text-[var(--color-text-muted)]">
                                                    {tabMessages.empty}
                                                </TableCell>
                                            </TableRow>
                                        )}
                        ...
                        {!loadingHistory && (
                            <Container unstyled className="flex flex-wrap items-center justify-between gap-2 text-sm text-[var(--color-text-secondary)]">
                                <Container unstyled className="flex items-center gap-2">
                                ...
                                <Container unstyled className="flex items-center gap-2">
                                    <Text as="span" className="text-xs">{tabMessages.page(safePage + 1, totalPages)}</Text>
                                    <Button
                                        type="button"
...
const headerCellClass = 'py-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--color-text-muted)]';
