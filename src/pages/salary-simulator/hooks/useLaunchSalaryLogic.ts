...
export function useLaunchSalaryLogic(
  ...
  const handleOpenLaunchSalaryDialog = () => {
    ...
    if (netPay <= 0) {
      setMessage({
        type: "error",
        text: "O lançamento só é permitido quando o valor líquido for maior que zero.",
      });
      return;
    }

    const defaultAccount =
      accounts.find((account) => account.name.toLowerCase() === "santander") ??
      accounts[0];
    const defaultCategory =
      incomeCategories.find(
        (category) => category.name.toLowerCase() === "salário trijay",
      ) ??
      incomeCategories.find(
        (category) => category.name.toLowerCase() === "salario trijay",
      ) ??
      incomeCategories[0];
    const now = new Date();
  ...
  const handleConfirmLaunchSalary = async () => {
    ...
    if (
      ...
      setMessage({
        type: "error",
        text: "Preencha descrição, conta, categoria e data da transação.",
      });
      return;
    }
    try {
      ...
      setMessage({
        type: "success",
        text: "Transação de salário lançada como pendente.",
      });
    } catch (error) {
