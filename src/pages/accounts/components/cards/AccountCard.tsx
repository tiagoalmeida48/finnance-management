...
export function AccountCard({ account, handleOpenMenu, cards, navigate }: AccountCardProps) {
    ...
    const balanceColorRef = useApplyElementStyles<HTMLParagraphElement>({
        color: account.current_balance >= 0 ? (account.color || 'var(--color-primary)') : 'var(--color-error)',
    });
            ...
            <CardContent className="p-6">
                <Row className="items-start justify-between">
                    <Stack ref={iconBgRef} className="mb-2 rounded-lg p-1.5">
                        {createElement(accountTypeIcon, { size: 24, color: account.color || 'var(--color-primary)' })}
                    </Stack>
                ...
                <Stack className="gap-1.5">
                    <Stack>
                        <Text className="block text-xs text-white/70">{accountCardMessages.initialBalanceLabel}</Text>
                    ...
                    <Stack>
                        <Text className="block text-xs text-white/70">{accountCardMessages.currentBalanceLabel}</Text>
