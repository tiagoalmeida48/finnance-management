...
export function SiteBrandingForm() {
    ...
    const handleSaveBranding = async () => {
        ...
        try {
            ...
            const text = error instanceof Error ? error.message : brandingMessages.errorSave;
            setMessage({ type: 'error', text });
        }
    };

    return (
        <Container unstyled className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-2">
            <Container unstyled className="mb-1.5 flex items-center gap-1 text-[var(--color-primary)]">
                <Palette size={16} />
            ...
            <Stack className="space-y-1.5">
                ...
                <Container unstyled className="space-y-1">
                    ...
                    <Container unstyled className="flex items-center gap-1.5">
                        <Container unstyled className="flex h-[52px] w-[52px] items-center justify-center overflow-hidden rounded-[10px] border border-white/[0.14] bg-white/[0.04]">
                            {logoImageInput ? (
                                <img src={logoImageInput} alt={brandingMessages.logoAlt} className="h-full w-full object-cover" />
                            ) : (
                                <Text className="font-bold text-white/70">{brandingMessages.logoPlaceholder}</Text>
                            )}
                        </Container>
