...
export function ProfileHeaderSection({ avatarUrl, uploading, uploadAvatar }: ProfileHeaderSectionProps) {
    ...
    return (
        <Container unstyled className="rounded-lg border border-white/[0.08] bg-white/[0.01] p-1.5 md:p-3.5">
            <Stack className="flex-row items-center justify-between gap-1.5">
                <Container unstyled className="relative">
                    <Container unstyled className="relative">
                        <Container unstyled className="h-16 w-16 overflow-hidden rounded-full border-[1.5px] border-[var(--color-primary)47] bg-[var(--color-primary)] text-[var(--color-neutral-black)] shadow-[0_6px_14px_var(--overlay-black-25)]">
                            {avatarUrl ? (
                                <img src={avatarUrl} alt={headerMessages.avatarAlt} className="h-full w-full object-cover" />
                            ) : null}
                        </Container>
                        <Container unstyled className="absolute bottom-0 right-0">
                            ...
                            >
                                ...
                                />
                                {uploading ? (
                                    <Text as="span" className="inline-block h-3 w-3 animate-spin rounded-full border border-current border-r-transparent" />
                                ) : (
                                    <Camera size={12} />
                                )}
                            </IconButton>
