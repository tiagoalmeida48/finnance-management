...
const formatSignedBRL = (val: number) =>
  val < 0 ? formatBRL(val).replace("R$", "R$ -") : formatBRL(val);
...
export function DashboardCashFlowChart({
  ...
  const cashFlowValues = chartData.flatMap((point) => [
    point.receita,
    point.despesa,
  ]);
  const maxCashFlowValue = cashFlowValues.length
    ? Math.max(...cashFlowValues)
    : 0;
  const yAxisDomainMax =
    maxCashFlowValue > 0 ? Math.ceil(maxCashFlowValue * 1.15) : 1000;
      ...
      <CardContent className="p-6">
          ...
          <Container unstyled className="flex items-center gap-4">
            <Container unstyled className="flex items-center gap-1">
              ...
              <Text className="text-xs text-[var(--color-text-secondary)]">
                {chartMessages.income}
              </Text>
            </Container>
            <Container unstyled className="flex items-center gap-1">
              <Container
                unstyled
                className="h-2 w-2 rounded-full bg-[var(--color-error)]"
              />
              <Text className="text-xs text-[var(--color-text-secondary)]">
                {chartMessages.expense}
              </Text>
            </Container>
          </Container>
        </Container>
        <Container unstyled className="h-[280px] w-full min-w-0">
          {hasInsufficientCashFlowData ? (
            <Container
              unstyled
              className="flex h-full items-center justify-center rounded-xl border border-dashed border-[var(--overlay-white-12)] bg-white/[0.02] px-4 text-center text-sm text-[var(--color-text-secondary)]"
            >
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(
                    ...
                    name?: string,
                  ) => [formatSignedBRL(Number(value) || 0), name || ""]}
                />
                <Line
                  type="monotone"
                  dataKey="receita"
