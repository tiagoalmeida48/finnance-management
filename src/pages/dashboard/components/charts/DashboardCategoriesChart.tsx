...
function CustomPieTooltip({
  ...
  const percentage =
    totalCategoryValue > 0
      ? (Number(item.value || 0) / totalCategoryValue) * 100
      : 0;
...
export function DashboardCategoriesChart({
  categories,
  ...
  const totalCategoryValue =
    pieCategories.reduce((sum, cat) => sum + cat.value, 0) || 1;

  const setCategoryProgressRef = (
  ...
  return (
    <Card className="h-full">
      <CardContent className="p-6">
        ...
        <Container
          unstyled
          className="flex h-[180px] w-full min-w-0 justify-center"
        >
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip
                content={
                  <CustomPieTooltip totalCategoryValue={totalCategoryValue} />
                }
                cursor={false}
                offset={16}
            ...
            return (
              ...
              >
                ...
                >
                  <Container
                    unstyled
                    className={`flex items-center gap-1.5 ${getPaletteClassByFill(cat.fill)}`}
                  >
                    <Container
                      unstyled
                      className="h-2.5 w-2.5 rounded-[3px] bg-current"
                    />
                  ...
                  >
                    <Container
                      unstyled
                      className="h-1 w-[60px] overflow-hidden rounded bg-white/5"
                    >
                      <Container
                        unstyled
                        ref={(node) =>
                          setCategoryProgressRef(node, percentage, idx)
                        }
