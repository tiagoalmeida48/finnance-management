...
export function DashboardFilters({
    ...
    return (
        <Row className="items-center gap-2">
            <label className="sr-only" htmlFor="dashboard-year">{filtersMessages.yearLabel}</label>
            <Container unstyled className="rounded-md border border-[var(--color-border)] bg-white/[0.03] px-2 py-1">
                <select
                    id="dashboard-year"
                ...
                >
                    {years.map((year) => (
                        <option key={year} value={year} className="bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]">
                            {year}
                        </option>
                    ))}
                </select>
