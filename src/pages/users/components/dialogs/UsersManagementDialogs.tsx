    ...
    return (
        <>
            <FormDialog
                ...
                actions={(
                    <>
                        <Button onClick={() => setCreateOpen(false)} disabled={saving}>{messages.common.actions.cancel}</Button>
            ...
            >
                <Stack className="mt-0.5 gap-1.5">
                    <FormField htmlFor="user-create-full-name" label={dialogMessages.fullNameLabel}>
                        <Input id="user-create-full-name" value={fullName} onChange={(e) => setFullName(e.target.value)} />
                    </FormField>
                    <FormField htmlFor="user-create-email" label={dialogMessages.emailLabel}>
                        <Input id="user-create-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
                    </FormField>
                    <FormField htmlFor="user-create-password" label={dialogMessages.initialPasswordLabel}>
                        <Input id="user-create-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
                    </FormField>
                    <Row className="items-center gap-1">
                        <label htmlFor="user-create-admin" className="text-sm font-medium text-[var(--color-text-secondary)]">
            ...
            <FormDialog
                ...
                actions={(
                    <>
                        <Button onClick={() => setEditTarget(null)} disabled={saving}>{messages.common.actions.cancel}</Button>
            ...
            >
                <Stack className="mt-0.5 gap-1.5">
                    <FormField htmlFor="user-edit-full-name" label={dialogMessages.fullNameLabel}>
                        <Input id="user-edit-full-name" value={fullName} onChange={(e) => setFullName(e.target.value)} />
                    </FormField>
                    <FormField htmlFor="user-edit-email" label={dialogMessages.emailLabel}>
                        <Input id="user-edit-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
                    </FormField>
                    <Row className="items-center gap-1">
                        <label htmlFor="user-edit-admin" className="text-sm font-medium text-[var(--color-text-secondary)]">
                            {dialogMessages.adminUserLabel}
                        </label>
                        <input
                            id="user-edit-admin"
            ...
            <FormDialog
                ...
                actions={(
                    <>
                        <Button onClick={() => setPasswordTarget(null)} disabled={saving}>{messages.common.actions.cancel}</Button>
            ...
            <Dialog open={Boolean(deleteTarget)} onClose={() => !saving && setDeleteTarget(null)}>
                ...
                <DialogActions>
                    <Button onClick={() => setDeleteTarget(null)} disabled={saving}>{messages.common.actions.cancel}</Button>
