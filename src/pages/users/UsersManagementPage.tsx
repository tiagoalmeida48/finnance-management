                        ...
                        <CardContent className="p-[18px] md:p-[22px]">
                            <Container unstyled className="space-y-2">
                                ...
                                <CollectionState
                                    ...
                                    emptyFallback={(
                                        <Container unstyled className="rounded-lg border border-dashed border-[var(--color-border)] p-4 text-center text-sm text-[var(--color-text-secondary)]">
                                            {pageMessages.empty}
                                        </Container>
                                    )}
                                >
                                        ...
                                        <Table className="w-full text-left text-sm">
                                            <TableHead>
                                                <TableRow className="border-b border-white/10">
                                                    <TableHeaderCell className="border-b-0 px-2 py-2">{pageMessages.columns.name}</TableHeaderCell>
                                                    <TableHeaderCell className="border-b-0 px-2 py-2">{pageMessages.columns.email}</TableHeaderCell>
                                                    <TableHeaderCell className="border-b-0 px-2 py-2">{pageMessages.columns.admin}</TableHeaderCell>
                                                    <TableHeaderCell className="border-b-0 px-2 py-2 text-right">{pageMessages.columns.actions}</TableHeaderCell>
                                                </TableRow>
                                            </TableHead>
                                            <TableBody>
                                                {users.map((user) => (
                                                    <TableRow key={user.id} className="border-b border-white/5">
                                                        <TableCell className="border-b-0 px-2 py-2 text-[var(--color-text-secondary)]">{user.full_name || '-'}</TableCell>
                                                        <TableCell className="border-b-0 px-2 py-2 text-[var(--color-text-secondary)]">{user.email}</TableCell>
                                                        <TableCell className="border-b-0 px-2 py-2 text-[var(--color-text-secondary)]">{user.is_admin ? pageMessages.yes : pageMessages.no}</TableCell>
