    ...
    const sidebarContent = (
        ...
        >
            <div>
                <div className="flex min-h-[72px] items-center justify-start gap-1.5 border-b border-[var(--color-border)] p-2">
                    <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center overflow-hidden">
                        {logoImage ? (
                            <img src={logoImage} alt="Logo do site" className="h-full w-full object-cover" />
                        ) : (
                    ...
                    <p
                        className={`font-heading overflow-hidden whitespace-nowrap text-[15px] font-bold tracking-[-0.02em] text-[var(--color-text-primary)] transition-all duration-200 ${open ? 'w-auto translate-x-0 opacity-100' : 'w-0 -translate-x-2 opacity-0'
                            }`}
                    >
                        {siteTitle}
                    </p>
            ...
            <div className="border-t border-[var(--color-border)] px-1 py-1.5">
                ...
                >
                    <div
                        className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[10px] bg-[linear-gradient(135deg,var(--color-info)_0%,var(--color-secondary)_100%)] text-sm font-bold text-white"
                    >
                        {profile?.avatar_url ? (
                            <img src={profile.avatar_url} alt="Avatar" className="h-full w-full rounded-[10px] object-cover" />
                        ) : (
                ...
                >
                    ...
                    <span className={`whitespace-nowrap text-[13px] font-semibold transition-opacity duration-200 ${open ? 'opacity-100' : 'opacity-0'}`}>
                        Sair
                    </span>
                </button>
            </div>
