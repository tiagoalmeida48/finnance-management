...
import {
    ...
    MenuItem, FormControl, InputLabel, Select, FormHelperText, Box
} from '@mui/material';
...
export function CardFormModal({ open, onClose, card }: CardFormModalProps) {
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
            <DialogTitle sx={{ fontWeight: 700 }}>{card ? 'Editar Cartão' : 'Novo Cartão'}</DialogTitle>
            <form onSubmit={handleSubmit(onSubmit)}>
                <DialogContent>
                    <Stack spacing={3} sx={{ mt: 1 }}>
                        <TextField fullWidth label="Nome do Cartão" {...register('name')} error={!!errors.name} helperText={errors.name?.message} />

                        <FormControl fullWidth error={!!errors.bank_account_id}>
                            <InputLabel>Conta Vinculada</InputLabel>
                            <Controller
                                name="bank_account_id"
                                control={control}
                                render={({ field }) => (
                                    <Select {...field} label="Conta Vinculada">
                                        {accounts?.map((acc) => (
                        ...
                        <TextField fullWidth label="Limite de Crédito" type="number" {...register('credit_limit', { valueAsNumber: true })} InputProps={{ startAdornment: <Box sx={{ mr: 1, color: 'text.secondary' }}>R$</Box> }} />

                        <Stack direction="row" spacing={2}>
                            <TextField fullWidth label="Dia Fechamento" type="number" {...register('closing_day', { valueAsNumber: true })} />
                            <TextField fullWidth label="Dia Vencimento" type="number" {...register('due_day', { valueAsNumber: true })} />
                        </Stack>
                        ...
                        <Stack direction="row" alignItems="center" spacing={2}>
                            ...
                            <Box>
                                ...
                                <Typography variant="caption" color="text.secondary">Destaque na listagem</Typography>
                            </Box>
                        </Stack>
                    </Stack>
                </DialogContent>
                <DialogActions sx={{ p: 3, pt: 0 }}>
                    ...
                    <Button variant="contained" type="submit" disabled={createCard.isPending || updateCard.isPending}>Salvar</Button>
                </DialogActions>
            </form>
        </Dialog>
    );
