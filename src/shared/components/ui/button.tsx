  ...
  ],
  {
    variants: {
      variant: {
        ...
        secondary: 'bg-[var(--color-surface)] text-[var(--color-text-primary)] hover:bg-[var(--color-card-hover)]',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, color, size, fullWidth, startIcon, endIcon, component, children, ...props }, ref) => {
    const Comp = (component ?? 'button') as React.ElementType

    const resolvedVariant = (() => {
      if (variant === 'contained') return color === 'error' ? 'destructive' : 'default'
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant: resolvedVariant, size: resolvedSize, fullWidth }), className)}
        ref={ref}
        {...props}
      >
