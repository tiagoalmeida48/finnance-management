...
export const salarySettingsService = {
  ...
  async createSettingWithValidity(input: CreateSalarySettingInput) {
    ...
    if (
      currentOpenSetting &&
      normalizedStartDate <= currentOpenSetting.date_start
    ) {
      throw new Error(
        `A data de inicio deve ser maior que ${currentOpenSetting.date_start}.`,
      );
    }
    ...
    try {
      if (currentOpenSetting) {
        const previousEndDate = formatDate(
          subDays(parseISO(normalizedStartDate), 1),
        );
  ...
  async updateSetting(input: UpdateSalarySettingInput) {
    ...
    const hasOverlap = (rows ?? []).some((row) => {
      const isSameRow =
        row.date_start === originalStartDate &&
        row.date_end === originalEndDate;
    ...
    if (hasOverlap) {
      throw new Error(
        "A vigencia informada sobrepoe outro periodo ja existente.",
      );
    }
  ...
  async deleteCurrentSettingAndRestorePrevious() {
    ...
    try {
      ...
      if (stillOpenError && stillOpenError.code !== "PGRST116")
        throw stillOpenError;
      if (stillOpenSetting) {
        throw new Error(
