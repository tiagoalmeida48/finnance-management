...
export const cardsService = {
  async getAll() {
    ...
    return cardRows.map((card) => {
      ...
      const unpaidInvoices = cardInvoices.filter(
        (inv) => inv.status !== "paid",
      );
      const totalUsage = unpaidInvoices.reduce(
        (acc, inv) =>
          acc + (Number(inv.total_amount) - Number(inv.paid_amount)),
        0,
      );

      const currentInvoiceData = cardInvoices.find(
        (inv) => inv.month_key === currentMonthKey,
      );
      const currentInvoice = currentInvoiceData
        ? Number(currentInvoiceData.total_amount) -
          Number(currentInvoiceData.paid_amount)
        : 0;
  ...
  async createStatementCycle(input: CreateCreditCardStatementCycleInput) {
      ...
      const { data: seededCycle, error: seedError } = await supabase
        ...
        .insert({
          ...
          notes: input.notes?.trim()
            ? input.notes.trim()
            : "Vigencia inicial criada manualmente.",
        })
        .select("*")
        .single();
    ...
    if (normalizedStartDate < firstCycle.date_start) {
      const newEnd = format(
        subDays(parseISO(firstCycle.date_start), 1),
        "yyyy-MM-dd",
      );
      ...
      await invoicesService.reprocessInvoicesFromDate(
        input.card_id,
        normalizedStartDate,
      );
  ...
  async getCardDetails(id: string) {
    ...
    const cardCycles = sortStatementCyclesAsc(
      (cycles ?? []) as CreditCardStatementCycle[],
    );
