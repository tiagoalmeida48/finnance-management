...
import { replaceDateDayPreservingMonth, toDateKeyIgnoringTime } from '@/shared/utils/transactionsGroup.utils';
import { linkTransactionToInvoice, recalculateInvoiceTotal, recalculateInvoicesForTransactions } from '../invoice-reconciliation.service';
...
export const transactionsBatchService = {
    async batchCreate(transactions: Partial<Transaction>[]) {
        ...
        const hasComplexRules = createPayloads.some((transaction) => {
            ...
            return Boolean(transaction.is_installment) || totalInstallments > 1 || (transaction.is_fixed && repeatCount > 1);
        });

        if (hasComplexRules) {
            return await Promise.all(createPayloads.map((transaction) => transactionsCreationService.create(transaction)));
        }

        const userId = await requireAuthenticatedUserId();
        const rowsToInsert = createPayloads.map((transaction) => sanitizeCreatePayload({
            ...transaction,
            user_id: userId,
    ...
    async batchPay(ids: string[], accountId: string, paymentDate: string) {
        ...
        const previousTransactions = (previousTransactionsRaw ?? []) as Transaction[];
    ...
    async batchChangeDay(ids: string[], day: number) {
        ...
        const previousTransactionsById = new Map(transactions.map((transaction) => [transaction.id, transaction]));
        ...
        await Promise.all(Array.from(affectedOldInvoiceIds).map((invoiceId) => recalculateInvoiceTotal(invoiceId)));
        return updatedTransactions;
    },
