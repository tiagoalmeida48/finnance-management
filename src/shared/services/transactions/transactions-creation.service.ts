...
import { shiftDateByMonths, stripInstallmentSuffix, toDateKeyIgnoringTime } from '@/shared/utils/transactionsGroup.utils';
...
const createSingleTransaction = async (userId: string, transaction: CreateTransactionData) => {
...
const createInstallmentTransactions = async (
    ...
    for (let installmentIndex = 1; installmentIndex <= totalInstallments; installmentIndex += 1) {
        ...
        const installmentAmount = transaction.installment_amounts?.[installmentIndex - 1] ?? transaction.amount;

        transactionsToCreate.push(sanitizeCreatePayload({
            ...transaction,
            user_id: userId,
            description: installmentDescription,
            amount: installmentAmount,
            payment_date: shiftDateByMonths(transaction.payment_date, installmentIndex - 1)
                ?? toDateKeyIgnoringTime(transaction.payment_date)
                ?? transaction.payment_date,
            purchase_date: shiftDateByMonths(transaction.purchase_date ?? undefined, installmentIndex - 1)
...
const createRecurringTransactions = async (
    ...
    for (let repeatIndex = 0; repeatIndex < repeatCount; repeatIndex += 1) {
        transactionsToCreate.push(sanitizeCreatePayload({
            ...transaction,
            user_id: userId,
            payment_date: shiftDateByMonths(transaction.payment_date, repeatIndex)
                ?? toDateKeyIgnoringTime(transaction.payment_date)
                ?? transaction.payment_date,
            purchase_date: shiftDateByMonths(transaction.purchase_date ?? undefined, repeatIndex)
                ?? toDateKeyIgnoringTime(transaction.purchase_date ?? undefined)
                ?? transaction.purchase_date
                ?? undefined,
...
export const transactionsCreationService = {
    async create(transaction: CreateTransactionData) {
        ...
        const repeatCount = transaction.is_fixed ? normalizeToPositiveInteger(transaction.repeat_count ?? 1, 1) : 1;
        const isInstallment = Boolean(transaction.is_installment) || totalInstallments > 1;
        const userId = await requireAuthenticatedUserId();

        if (isInstallment && totalInstallments > 1) {
            return await createInstallmentTransactions(userId, transaction, totalInstallments);
        }

        if (transaction.is_fixed && repeatCount > 1) {
            return await createRecurringTransactions(userId, transaction, repeatCount);
        }
