...
const createSingleTransaction = async (userId: string, transaction: CreateTransactionData) => {
  ...
  const createdTransaction = TransactionSchema.parse(data);
...
const createInstallmentTransactions = async (
  ...
  for (let installmentIndex = 1; installmentIndex <= totalInstallments; installmentIndex += 1) {
    ...
    const installmentAmount =
      transaction.installment_amounts?.[installmentIndex - 1] ?? transaction.amount;
    ...
    transactionsToCreate.push(
      sanitizeCreatePayload({
        ...
        purchase_date:
          shiftDateByMonths(transaction.purchase_date ?? undefined, installmentIndex - 1) ??
    ...
    transactionsToCreate.push(
      sanitizeCreatePayload({
        ...
        payment_date:
          ...
          transaction.payment_date,
        purchase_date:
          shiftDateByMonths(transaction.purchase_date ?? undefined, repeatIndex) ??
          toDateKeyIgnoringTime(transaction.purchase_date ?? undefined) ??
          transaction.purchase_date ??
          undefined,
