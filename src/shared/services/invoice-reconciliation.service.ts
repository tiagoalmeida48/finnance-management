...
const buildClosingAndDueDate = (monthKey: string, closingDayInput: number, dueDayInput: number) => {
  ...
  const closingBase =
    closingDay >= dueDay ? new Date(dueYear, dueMonth - 1, 1) : new Date(dueYear, dueMonth, 1);
...
export const reprocessInvoicesFromDate = async (cardId: string, fromDateInput: string) => {
  ...
  await Promise.all(
    Array.from(affectedInvoiceIds).map((invoiceId) => recalculateInvoiceTotal(invoiceId)),
  );
