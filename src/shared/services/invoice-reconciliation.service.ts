...
type InvoiceTransaction = Pick<
    Transaction,
    'id' | 'user_id' | 'card_id' | 'invoice_id' | 'amount' | 'type' | 'is_paid' | 'payment_date' | 'purchase_date'
>;
...
const buildClosingAndDueDate = (monthKey: string, closingDayInput: number, dueDayInput: number) => {
...
const toUniqueInvoiceIds = (transactions: Array<{ invoice_id?: string | null }>) => (
    Array.from(new Set(transactions.map((transaction) => transaction.invoice_id).filter(Boolean) as string[]))
);
...
export const linkTransactionToInvoice = async (transaction: InvoiceTransaction) => {
    ...
    const resolved = resolveStatementMonth(
        { purchase_date: transaction.purchase_date, payment_date: transaction.payment_date },
...
export const reprocessInvoicesFromDate = async (cardId: string, fromDateInput: string) => {
    ...
    for (const invoice of existingInvoicesRaw ?? []) {
        invoicesByMonthKey.set(String(invoice.month_key), invoice as Record<string, unknown>);
    }
    ...
    for (const [monthKey, cycle] of neededInvoicesByMonthKey.entries()) {
        ...
        if (createError) throw createError;
        if (createdInvoice) invoicesByMonthKey.set(monthKey, createdInvoice as Record<string, unknown>);
    }
    ...
    for (const [invoiceId, transactionIds] of transactionIdsByInvoiceId.entries()) {
        const { error } = await supabase
            .from('transactions')
    ...
    await Promise.all(Array.from(affectedInvoiceIds).map((invoiceId) => recalculateInvoiceTotal(invoiceId)));
    ...
    const usedInvoiceIds = new Set(
        (invoicesWithTransactions ?? []).map((row) => row.invoice_id as string | null).filter(Boolean) as string[],
    );
    const emptyInvoiceIds = invoiceIds.filter((invoiceId) => !usedInvoiceIds.has(invoiceId));
...
export const recalculateInvoicesForTransactions = async (transactions: Array<{ invoice_id?: string | null }>) => {
